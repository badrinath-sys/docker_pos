...
<main class="py-4 bg-surface-secondary">
    <form action="{{route('admin.search')}}" method="get">
        <div class="input-group relative flex flex-wrap items-stretch w-1/2 mb-4 ml-6">
            <input type="search" class="form-control relative flex-auto min-w-0 block w-1/2 px-3 py-2 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" placeholder="Search" aria-label="Search" aria-describedby="button-addon2" name="search">
        </div> @if(!empty($item))
        @foreach($item as $list) @if($list->image!='') <a onclick="toggleModal1()">
                                    ...
                                    <table class="table table-hover table-nowrap">
                                        ...
                                        @if(session('cart')) 
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="text-heading font-semibold" href="#"> <img width="50px" src="{{asset('storage/media/'.$details['image'])}}" /> </a>
                                                </td>
                                                <td> ₹{{ $details['price'] }} </td>
                                                <td>
                                                    <div class="flex flex-row h-6 w-10 rounded relative bg-transparent mt-1">
                                                        <input type="number" class="outline-none focus:outline-none text-center w-full bg-white 
                                                                border border-solid border-gray-300 rounded font-normal text-sm hover:text-black focus:text-black  md:text-basecursor-default flex items-center text-gray-700  outline-none" name="custom-input-number" value="{{ $details['quantity'] }}" />
                                                    </div>
                                                </td>
                                ...
                                <div class="fixed z-10 overflow-y-auto top-0 w-full left-0 hidden" id="modal">
                                    <div class="flex items-center justify-center min-height-100vh pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                                        ...
                                        <div class="inline-block align-center bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                                            <form action="{{url('admin/checkout')}}" method="post"> {{ csrf_field() }}
                                                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                                    <label>Mobile Number</label>
                                                    <input type="text" class="w-full bg-gray-100 p-2 mt-2 mb-3 border border-solid border-gray-300 rounded " name="mobile" />
                                                    <label>User Name</label>
                                                    <input type="text" class="w-full bg-gray-100 p-2 mt-2 mb-3 border border-solid border-gray-300 rounded  " name="name" />
                                                    <label>Amount</label>
                                                    <input type="text" class="w-full bg-gray-100 p-2 mt-2 mb-3 border border-solid border-gray-300 rounded  " value="₹{{$total}}" name="amount" />
                                                    <label>Payment</label>
                                          ...
                                          ease-in-out
                                          mt-2
                                          focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" name="payment_type" aria-label="Default select example">
